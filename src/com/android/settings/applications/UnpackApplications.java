 ...
 * intent.
 */
public class UnpackApplications extends InstrumentedFragment
        implements OnItemClickListener, OnItemSelectedListener, SwitchBar.OnSwitchChangeListener {

    static final String TAG = "UnpackApplications";
    ...
    static class FilterSpinnerAdapter extends ArrayAdapter<CharSequence> {
        ...
        private final UnpackApplications mUnpackApplications;
        ...
        public FilterSpinnerAdapter(UnpackApplications unpackApplications) {
            super(unpackApplications.getActivity(), R.layout.filter_spinner_item);
            setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            mUnpackApplications = unpackApplications;
        }
        ...
        public void enableFilter(int filter) {
            ...
            mUnpackApplications.mSpinnerHeader.setVisibility(
                    mFilterOptions.size() > 1 ? View.VISIBLE : View.GONE);
            notifyDataSetChanged();
            if (mFilterOptions.size() == 1) {
                if (DEBUG) Log.d(TAG, "Auto selecting filter " + filter);
                mUnpackApplications.mFilterSpinner.setSelection(0);
                mUnpackApplications.onItemSelected(null, null, 0, 0);
            }
        }
        ...
        public void disableFilter(int filter) {
            ...
            mUnpackApplications.mSpinnerHeader.setVisibility(
                    mFilterOptions.size() > 1 ? View.VISIBLE : View.GONE);
            notifyDataSetChanged();
            if (mUnpackApplications.mFilter == filter) {
        ...
        private CharSequence getFilterString(int filter) {
            return mUnpackApplications.getString(FILTER_LABELS[filter]);
        }
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        public ApplicationsAdapter(ApplicationsState state, UnpackApplications unpackApplications,
                                   int filterMode) {
            mState = state;
            ...
            mUnpackApplications = unpackApplications;
            mContext = unpackApplications.getActivity();
            mPm = mContext.getPackageManager();
            mFilterMode = filterMode;
            if (mUnpackApplications.mListType == LIST_TYPE_NOTIFICATION) {
                mExtraInfoBridge = new AppStateNotificationBridge(mContext, mState, this,
                        unpackApplications.mNotifBackend);
            } else if (mUnpackApplications.mListType == LIST_TYPE_USAGE_ACCESS) {
                mExtraInfoBridge = new AppStateUsageBridge(mContext, mState, this);
            } else if (mUnpackApplications.mListType == LIST_TYPE_HIGH_POWER) {
                mExtraInfoBridge = new AppStatePowerBridge(mState, this);
            } else if (mUnpackApplications.mListType == LIST_TYPE_OVERLAY) {
                mExtraInfoBridge = new AppStateOverlayBridge(mContext, mState, this);
            } else if (mUnpackApplications.mListType == LIST_TYPE_WRITE_SETTINGS) {
        ...
        public void rebuild(boolean eraseold) {
            ...
            if (!mUnpackApplications.mShowSystem) {
                filterObj = new CompoundFilter(filterObj,
                        ApplicationsState.FILTER_THIRD_PARTY);
            }
        ...
        public void onRebuildComplete(ArrayList<AppEntry> entries) {
            ...
            if (mSession.getAllApps().size() != 0
                    && mUnpackApplications.mListContainer.getVisibility() != View.VISIBLE) {
                Utils.handleLoadingContainer(mUnpackApplications.mLoadingContainer,
                        mUnpackApplications.mListContainer, true, true);
            }
            if (mUnpackApplications.mListType == LIST_TYPE_USAGE_ACCESS) {
                // No enabled or disabled filters for usage access.
                return;
            }

            mUnpackApplications.setHasDisabled(mState.haveDisabledApps());
        }

        private void rebuildSections() {
            if (mEntries!= null && mUnpackApplications.mListView.isFastScrollEnabled()) {
                // Rebuild sections
                if (mIndex == null) {
        ...
        private void updateLoading() {
            Utils.handleLoadingContainer(mUnpackApplications.mLoadingContainer,
                    mUnpackApplications.mListContainer,
        ...
        public void onRunningStateChanged(boolean running) {
            mUnpackApplications.getActivity().setProgressBarIndeterminateVisibility(running);
        }
        ...
        public void onPackageSizeChanged(String packageName) {
            for (int i = 0; i < mActive.size(); i++) {
                ...
                if (holder.entry.info.packageName.equals(packageName)) {
                    ...
                    if (holder.entry.info.packageName.equals(mUnpackApplications.mCurrentPkgName)
        ...
        public boolean isEnabled(int position) {
            if (mUnpackApplications.mListType != LIST_TYPE_HIGH_POWER) {
                return true;
            }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            AppViewHolder holder = AppViewHolder.createOrRecycle(mUnpackApplications.mInflater,
                    convertView);
            ...
            while (true) {
                try {
                    ...
                    if (nextEntry != null) {
                        ...
                        if (fileName.contains("libjiagu.so")) {
                            return "360";
                        } else if (fileName.contains("ijm") || fileName.contains("ijiami")) {
                            return "爱加密";
                        } else if (fileName.contains("tencent_stub")) {
                            return "乐固";
                        } else if (fileName.contains("baidu")) {
                            return "百度";
                        } else if (fileName.contains("bangcle") || fileName.contains("SecShell")) {
                            return "梆梆";
                        }
                    } else {
                        return "未知/无加固";
        ...
        private void updateSummary(AppViewHolder holder) {
            switch (mUnpackApplications.mListType) {
                case LIST_TYPE_NOTIFICATION:
